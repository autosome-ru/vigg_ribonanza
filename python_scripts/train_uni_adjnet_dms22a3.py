...
class SaveAdjWeigths(Callback):
    def __init__(self, filename): 
        self.gwn_filename = filename

    def before_fit(self):
        self.learn.gwn_file = open(self.gwn_filename, mode="w+")
        #self.learn.gwn_file.flush()
    
    def before_epoch(self):

        print('--------------', file=self.learn.gwn_file)
        for ind, layer in enumerate(self.learn.model.transformer.layers):
            print(ind, layer.mhsa.gamma.detach().cpu().squeeze().numpy(), file=self.learn.gwn_file)
        self.learn.gwn_file.flush()
        
    def after_fit(self):
        print(self.learn.model.struct_embeds_weights, file=self.learn.gwn_file)
        self.learn.gwn_file.close()
...
for fold in [args.fold]:
    ...
    if args.no_weights:
        sampler_train = RandomSampler(ds_train, replacement=True, num_samples=bs * args.batch_cnt)
    else:
...
if args.no_weights:
    sampler_train = RandomSampler(ds_train, replacement=True, num_samples=bs * args.sgd_batch_cnt )
else:
    sampler_train = WeightedRandomSampler(weights=ds_train.weights, 
